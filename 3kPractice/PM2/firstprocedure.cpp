    ...
    /*    QFile file("Log.txt");
        if ((file.exists())&&(file.open(QIODevice::ReadOnly))){
            while(!file.atEnd()){
                ...
                if(lst[0]=="ПСО" && (lst[3].toInt()<time || time == -1)){
                    numb=lst[2].toInt();
                    time=lst[3].toInt();
                }
                else if(lst[0]=="НПС"){
...
QChartView* Firstprocedure::CreateFirstDiagramm(){ //Диаграмма строящая ряд распределения времени получения
...
QChartView* Firstprocedure::CreateSecondDiagramm(){//Диаграмма строящая вероятности получения сообщения приемником
